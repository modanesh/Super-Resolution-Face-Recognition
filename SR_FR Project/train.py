...
def extractFaces(a_dir, cascade, folder, levelFace=False):
    if cascade == "1":
        faceCascade = cv2.CascadeClassifier(config.HAAR_FACE_CASCADE_FILE)
        eyeCascade = cv2.CascadeClassifier(config.HAAR_EYE_CASCADE_FILE)
    else:
        faceCascade = cv2.CascadeClassifier(config.LBP_FACE_CASCADE_FILE)
        eyeCascade = None
...
def trainRecognizer(db_folder, featureExtractionAlg, cascadeDetector, trainSize=config.DEFAULT_FACE_SIZE, showFaces=False, forceTrain=False):

    # FEATURE EXTRACTION ALGORITHM

    if featureExtractionAlg == "1":
        recognizer = cv2.face.createLBPHFaceRecognizer()
    elif featureExtractionAlg == "2":
        recognizer = cv2.face.createFisherFaceRecognizer()
    else:
        recognizer = cv2.face.createEigenFaceRecognizer()
        # recognizer = cv2.face.createEigenFaceRecognizer(10)
    ...
    for folder in folders:
        faces = extractFaces(db_folder, cascadeDetector, folder, True)
...
if __name__ == '__main__':
    recognizer = trainRecognizer('Resources/Train Set', "1", "1", showFaces=False, forceTrain=True)
